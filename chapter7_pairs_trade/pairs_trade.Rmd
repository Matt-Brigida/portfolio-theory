...
inputPanel(
    ...
    sliderInput("buy", "Buy Threshold", min = .5, max = 4, value = 2, step = .1, animate = FALSE),
    
    sliderInput("sell", "Sell Threshold", min = 0, max = 1.5, value = 0.5, step = .1, animate = FALSE)
)
...
dygraph( cbind(numSD, as.xts(position, order.by = index(numSD))) , main = paste0(colnames(returns.t)[1], " & ", colnames(returns.t)[2]), ylab = "# Standard Deviations")
})
```
...
For some pairs we'll see a the average aquared deviation during the trading periods is more than 30 standard deviations from the average during the estimation period.  This seems very unlikely given Chebyshev's inequality, which states $P(|X - \mu| \geq k\sigma) \leq \frac{1}{k^2}$ where $X$ is a random variable with finite mean $\mu$ and standard deviation $\sigma$, and $k$ is a real number. 
