...
#### GitHub: [GitHup Repository](https://github.com/Matt-Brigida/portfolio-theory)
...
When we get a signal to buy at time $t$, we buy the stock with the lower noormalized price, and short sell the stock with the higher price.  Of course, if we are using trade prices, as we do with daily data, we trade at time $t+1$ prices (whatever our time increment $\Delta t$ is).  Therefore we are never sure of the number of standard deviation at which we'll openthe trade.  If we were basing our signal on intraday bid/ask prices, then we would open and close our trades much closer to our thresholds.  
