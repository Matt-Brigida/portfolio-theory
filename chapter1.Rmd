...
This is more simple than it looks.  $f(r)dr$ is the same rectangle as $p_s$ in the discrete version.  The only different is $f(r)dr$ is in the limit where the base of the rectangle gets smaller and smaller.  Note, you won't need to know the distinctions between the descrete and continuous cases for what follows---I just thought you should see both equations in case they come up in some context later.
...
Very often in finance we assume returns are normally distributed.  This assumtion means, given a mean and variance, the distribution is completely determined.  That is, is an asset's returns are assumed normal, then all we need to fully describe the asset is its mean and variance.  This effectively assumes the values of [higher moments](https://en.wikipedia.org/wiki/Moment_\(mathematics\)#Higher_moments) (such as skewness and kurtosis).  
