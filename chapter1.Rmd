...
```{r riskReturnHistory, echo=FALSE}
library(shiny)
library(dygraphs)
library(quantmod)
renderPlot({
...
When we switch to a time series of historical returns, we assign each return observation as equally likely.  So if there are $T$ returns, then each has probability $\frac{1}{T}$.  Therefore, 
